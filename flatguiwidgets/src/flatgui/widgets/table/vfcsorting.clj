...
(defevolverfn sorting-header-should-evolve :should-evolve-header
  (let [ current old-should-evolve-header
         new (get-property component [] :mouse-down)]
      (and (not current) new)))

(defevolverfn sorting-mode-evolver :mode
  (let [ vfc-clicked (should-evolve-header component)
...
(defevolverfn sorting-degree-evolver :degree
...
(defaccessorfn apply-sorting [contentpane prev-header-ids header-id prev-row-order modes]
    ...
    (if (= :none mode)
      ...
      (let [ row-count (count prev-row-order)
             key-fn (fn [row-order-item]
                      (get-value-from-col contentpane row-order-item header-id))
             sub-ranges (if (nil? prev-header-ids)
                          [[0 row-count]]
                            (find-subranges
                              (for [screen-row (range 0 row-count)]
                                (map
                                  (fn [prev-header-id] (get-value-from-col
                                                         contentpane
...
(defaccessorfn apply-sorting-feature [contentpane prev-row-order modes]
  (apply-vf-by-degree contentpane :sorting apply-sorting prev-row-order modes))

; Params

(defwidget vfcsorting
