...
(ns ^{:doc "Scrollable panel widget. Contains pre-installed
  ...
  flatgui.widgets.scrollpanel (:use flatgui.awt
                                    flatgui.comlogic
                                    flatgui.base
                                    flatgui.theme
                                    flatgui.paint
...
(def SCROLL-BAR-THICKNESS-DEFAULT 0.125)


(defevolverfn scrollpanelcontent-clip-size-evolver :clip-size
...
(defevolverfn scrollpanelcontent-viewport-matrix-evolver :viewport-matrix
  (let [ reason ((:evolve-reason-provider component) (:id component))]
    ...
    (cond
      ...
      (let [v-scrollbar-h (y (get-property component [:v-scrollbar] :clip-size))
            v-scroller-y (mx-get (get-property component [:v-scrollbar :scroller] :position-matrix) 1 3)
      ...
      (let [ old-y (mx-y old-viewport-matrix)]
        ;
        ;@todo  keep range
        ;
        (mx-set old-viewport-matrix 1 3 (- old-y (* (:wheel-rotation-step-y component) (get-wheel-rotation component)))))
      :else old-viewport-matrix)))


(defwidget "scrollpanelcontent"
  (array-map
    :position-matrix (transtation-matrix (flatgui.awt/px) (flatgui.awt/px) 0)
    :content-size (defpoint 14 12 0)
    :wheel-rotation-step-y 1
    :evolvers (array-map
                :clip-size scrollpanelcontent-clip-size-evolver
                :viewport-matrix scrollpanelcontent-viewport-matrix-evolver
                ))
  panel)

(defevolverfn verticalscroller-position-matrix-evolver :position-matrix
  (cond
...
;      (flatgui.widgets.scrollbar/scroller-position-matrix-evolver component)
    (= [:_ :content-pane] ((:evolve-reason-provider component) (:id component)))
      (let [ v-scroller-h (y (get-property component [:this] :clip-size))
             ...
             new-v-scroller-y (if (> (y content-extra-size) 0) (- (* (/ (mx-y content-viewport-matrix) (y content-extra-size)) (- v-scrollbar-h v-scroller-h))) 0)
             ;new-v-scroller-y (- (* (/ (mx-y content-viewport-matrix) (y content-extra-size)) (- v-scrollbar-h v-scroller-h)))

             ;repaint-reason ((:evolve-reason-provider component) (:id component))
             ;_ (println "--- non-mouse-event --> new-v-scroller-y = " new-v-scroller-y " content viewport y: " (mx-y content-viewport-matrix)
             ;    " repaint-reason " repaint-reason " old y " (mx-y old-position-matrix) " new y " new-v-scroller-y)
             ]
        (mx-set old-position-matrix 1 3 new-v-scroller-y))
    :else
      ;old-position-matrix
    (flatgui.widgets.scrollbar/scroller-position-matrix-evolver component)
      ))

(defevolverfn horizontalscroller-position-matrix-evolver :position-matrix
  (cond
;    (is-mouse-event? component)
;    (flatgui.widgets.scrollbar/scroller-position-matrix-evolver component)
    (= [:_ :content-pane] ((:evolve-reason-provider component) (:id component)))
    (let [ h-scroller-w (x (get-property component [:this] :clip-size))
           ...
           new-h-scroller-x (if (> (x content-extra-size) 0) (- (* (/ (mx-x content-viewport-matrix) (x content-extra-size)) (- h-scrollbar-w h-scroller-w))) 0)
           ;new-h-scroller-x (- (* (/ (mx-x content-viewport-matrix) (x content-extra-size)) (- h-scrollbar-w h-scroller-w)))

           ;repaint-reason ((:evolve-reason-provider component) (:id component))
           ;_ (println "--- non-mouse-event --> new-v-scroller-y = " new-v-scroller-y " content viewport y: " (mx-y content-viewport-matrix)
           ;    " repaint-reason " repaint-reason " old y " (mx-y old-position-matrix) " new y " new-v-scroller-y)
           ]
      (mx-set old-position-matrix 0 3 new-h-scroller-x))
    :else
    ;old-position-matrix
    (flatgui.widgets.scrollbar/scroller-position-matrix-evolver component)
    ))
...
;(defwidget "scrollpanelverticalscroller"
;  {:evolvers {:position-matrix verticalscroller-position-matrix-evolver}}
;  scroller)

(defwidget "scrollpanelverticalbar"
  (array-map
    :scroll-bar-thickness SCROLL-BAR-THICKNESS-DEFAULT
    :orientation :vertical
    :evolvers { :clip-size (accessorfn (defpoint
                                         ...
                                         0))
                :content-size (accessorfn (:clip-size component))
                :position-matrix (accessorfn (mx-set
                                               IDENTITY-MATRIX
                                               0 3
...
;                {:scroller scrollpanelverticalscroller})
    :children {:scroller (defcomponent scroller :scroller
                           {:evolvers {:position-matrix verticalscroller-position-matrix-evolver}
                            })}
...
(defwidget "scrollpanelhorizontalbar"
  (array-map
    :scroll-bar-thickness SCROLL-BAR-THICKNESS-DEFAULT
    ...
    :evolvers { :clip-size (accessorfn (defpoint
                                         ...
                                         0))
                :content-size (accessorfn (get-property component [:this] :clip-size))
                :position-matrix (accessorfn (mx-set
                                               IDENTITY-MATRIX
                                               1 3
...
(defwidget "scrollpanel"
  {
    ;:background :light
    :children (array-map
                :v-scrollbar (defcomponent scrollpanelverticalbar :v-scrollbar {})
                :h-scrollbar (defcomponent scrollpanelhorizontalbar :h-scrollbar {})
                :content-pane (defcomponent scrollpanelcontent :content-pane {})
                )
    }
  panel)
